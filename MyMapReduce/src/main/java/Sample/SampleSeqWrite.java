 ...
 **/
public class SampleSeqWrite extends Configured implements Tool {
    public static void main(String[] args) {
        try {
            System.exit(ToolRunner.run(
                    new SampleSeqWrite(), args));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//        SequenceFile.Writer.Option key = SequenceFile.Writer.keyClass(IntWritable.class);
        SequenceFile.Writer.Option key = SequenceFile.Writer.keyClass(Text.class);
        // 设置写入value类型
...
//        SequenceFile.Writer writer = SequenceFile.createWriter(conf, file, key);

        // 写入数据
//        writer.append(new IntWritable(20), NullWritable.get());
//        writer.append(new IntWritable(40), NullWritable.get());
        writer.append(new Text("lisi"), new Text("lisi"));
        writer.append(new Text("hhp"), new Text("hhp"));
        writer.append(new Text("wangwu"), new Text("wangwu"));
        writer.append(new Text("zhaoliu"), new Text("lisi"));
        writer.append(new Text("zhangsan"), new Text("lisi"));
        writer.append(new Text("sunqian"), new Text("lisi"));
        writer.append(new Text("zhaoyi"), new Text("lisi"));
        writer.append(new Text("liuzi"), new Text("lisi"));
        writer.append(new Text("abc"), new Text("lisi"));
        writer.append(new Text("dcfe"), new Text("lisi"));
        writer.append(new Text("tyhu"), new Text("lisi"));
